    ...
    public class ConnectionManager
    {
        ...
        public void SendWebSocketRequest(int websocketVersion)
        {
            ...
            while (offset < HandShakeRequest.Length)
            {
                ...
                TestUtility.LogTrace("Client {0:D3}: Write {1} bytes: {2} ", Client.Id, outputData.Length,
        ...
        public void ReadDataCallback(IAsyncResult result)
        {
            ...
            if (bytesRead > 0)
            {
                ...
                {
                    TestUtility.LogTrace("ReadDataCallback: Client {0:D3}: Read Type {1} : {2} ", Client.Id, frame.FrameType, bytesReadIntotal);
        ...
        public Frame ReadData()
        {
            ...
            if (result != null)
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    TestUtility.LogTrace("Client {0:D3}: Read Type {1} : {2} ", Client.Id, frame.FrameType, frame.Content.Length);
                }
        ...
        public Frame Send(byte[] outputData)
        {
            ...
            {
                var result = Client.Stream.BeginWrite(outputData, 0, outputData.Length, WriteCallback, Client);
                TestUtility.LogTrace("Client {0:D3}: Write Type {1} : {2} ", Client.Id, frame.FrameType,
